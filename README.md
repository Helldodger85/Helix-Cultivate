...
- Environmental Learning's confidence-weighted model is a genuine fitted multi-variable regression (numpy-based OLS with prediction-interval confidence), but its output isn't yet wired into any live setpoint decision — the model fits and predicts correctly, nothing in the control loop consults it yet.
