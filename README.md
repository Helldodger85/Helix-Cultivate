...
- **Conditioning Room's own Humidity Setpoint** — a genuine, adjustable target (one flat value; Conditioning Room has no stage concept), with its dehumidifier/humidifier driven exclusively by its own dedicated RH sensor against it — never by Primary Grow Space's leaf VPD, which has nothing to do with Conditioning Room's actual humidity.
...
- **Temporary Override** — on Primary Grow Space, a Day/Night-aware set of sliders (Temp Setpoint, VPD Target, and — Day context only — Light Intensity, since the grow light is forced off outside its scheduled on-window regardless of override) for nudging today's targets without touching your saved stage settings; clearly indicated whenever active, persists across a Day/Night Smooth-Glides flip within the same stage, and automatically clears the moment the stage actually changes.
- **Explicit cycle lifecycle** — a cycle is either not-started or active; starting one and closing out a harvest are deliberate actions rather than an always-on implicit state. Every stage's full configuration is always visible and editable regardless of which state applies — a compact status panel at the top of the Plant Cycle tab is the only part that changes (the Start New Cycle form, or the live Day X/Y progress and Abort Cycle), so you can view, plan, and tune every stage's targets before ever starting a cycle, not only afterward.
...
- **Shadow Mode** — on by default whenever Environmental Learning is first enabled: the regression keeps learning and predicting every tick, wired into Conditioning Room's weather-feedforward setpoint pre-compensation, but nothing it predicts is actually applied to a real setpoint until you turn Shadow Mode off. A live comparison chart (real vs. shadow-predicted temperature, with a correlated weather-event log beneath it) and a current-moment "Shadow suggestion" readout let you watch what it would have done first; the Settings tab separately shows a trailing 7-day agreement summary.
...
- Frontend (dashboard panel) behavior has *some* real, automated behavioral coverage as of v1.5.1-v1.6.0: a dependency-free Node harness (`tests/js/harness.js`) evaluates the actual shipped `helix-panel.js`, instantiates its real custom elements, and inspects their genuinely rendered markup — currently covering the Plant Cycle tab's status-panel/timeline split and Growth Mode lock, the Temporary Override section's Day/Night slider visibility, the Progression Mode and Recipe Sharing explanatory copy, and the Shadow Mode toggle, comparison chart, and retrospective summary. It's real output verification, not just code review, but it has no actual DOM query engine (element lookups are inert stubs, not matched elements from the rendered tree), so it can't simulate real clicks or input events, and it's neither a real browser nor a live Home Assistant instance. The rest of the dashboard panel — hardware-mapping forms, the other zone tabs, Journal — is still verified only through code review and syntax checking, not this harness. The `pytest` suite (`tests/`) covers backend logic (coordinator, climate engine, config migrations, entity platforms) plus these frontend checks via subprocess.
- The Breeze engine's random re-modulation interval is roughly 60-90 seconds; there's no UI to tune this interval yet.
- Environmental Learning's confidence-weighted regression is wired into Conditioning Room's weather-feedforward setpoint pre-compensation, but ships with Shadow Mode on by default — it predicts and its prediction is logged every tick, but nothing it predicts touches a real setpoint until you deliberately turn Shadow Mode off on the Settings tab, after watching its predictions against reality on the comparison chart.
...
Current and historical issues are tracked on [GitHub Issues](https://github.com/Helldodger85/Helix-Cultivate/issues) — that's the authoritative list, not a static snapshot here that would inevitably go stale.
