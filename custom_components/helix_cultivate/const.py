...
# ── Zone occupancy (v1.4.0 Part 4) — decoupled from cycle_state ──────────────
# Answers only "is there currently plant material physically in this space",
# never "is a batch still being tracked somewhere". Primary Grow Space's flag
# is set True by start_cycle() and False only by space_now_empty() (which
# only exists when a dedicated Drying Room is configured) — with no
# dedicated Drying Room, drying happens in the same tent and this flag stays
# True for the whole cycle, closing off the exact scenario that could
# otherwise let a dependent zone (e.g. Conditioning Room) destabilize a
# curing harvest. Drying Room's flag clears only at harvest_complete() for
# that specific cycle_id, never at stage-tracking completion alone.
CONF_ZONE2_OCCUPIED: str = "zone2_occupied"
CONF_DRYING_OCCUPIED: str = "drying_occupied"
DEFAULT_ZONE2_OCCUPIED: bool = False
DEFAULT_DRYING_OCCUPIED: bool = False

# Cycle identity — CONF_ZONE2_CYCLE_ID is (re)generated by start_cycle() each
# time a new cycle begins in Primary Grow Space; CONF_DRYING_CYCLE_ID is the
# cycle_id currently occupying the dedicated Drying Room (set by
# space_now_empty(), cleared by harvest_complete() for that batch). The two
# can be genuinely different values at once — a fresh cycle_id germinating
# in a just-freed Primary Grow Space while an older cycle_id is still
# curing in Drying, both open concurrently.
CONF_ZONE2_CYCLE_ID: str = "zone2_cycle_id"
CONF_DRYING_CYCLE_ID: str = "drying_cycle_id"

# ── Cross-zone dependency flags (v1.4.0 Part 3) — explicit, never silently
# inferred. Gate whether Conditioning Room is eligible to run Deep
# Calibration: it may only do so when every zone flagged as depending on it
# is currently unoccupied (Part 3.2). Default suggestion logic lives in the
# frontend (auto-suggests based on what's actually mapped); the persisted
# value itself always reflects an explicit, confirmed grower choice.
CONF_ZONE2_DEPENDS_ON_CONDITIONING: str = "zone2_depends_on_conditioning"
CONF_DRYING_DEPENDS_ON_CONDITIONING: str = "drying_depends_on_conditioning"
# Conservative default — the cost of wrongly assuming independence is a
# ruined harvest, so "depends on Conditioning Room" is the safe fallback
# whenever a grower hasn't yet made an explicit choice.
DEFAULT_DEPENDS_ON_CONDITIONING: bool = True
...
# Opt-in, advanced layer — off by default for every install. When off: zero
# background logging, zero scheduled tests, zero influence on control, and
# the entire Settings tab is absent (not greyed out).
CONF_THERMAL_LEARNING_ENABLED: str = "thermal_learning_enabled"
DEFAULT_THERMAL_LEARNING_ENABLED: bool = False

# Two visible states — Learning runs for a fixed, configurable duration
# (independent of cycle_state/occupancy, since this must work with no grow
# happening at all) then graduates to Active unconditionally. The model
# keeps refitting indefinitely after graduating — it never "locks."
LEARNING_STATE_LEARNING: str = "learning"
LEARNING_STATE_ACTIVE: str = "active"
CONF_LEARNING_STATE: str = "thermal_learning_state"
...
# reaches full weight; below that, weight scales linearly with sample count
# so a first-ever extreme event can't be treated as confidently-known.
LEARNING_CONFIDENT_SAMPLE_COUNT: int = 20

# Deep Calibration (7.4) — deliberately cuts an actuator's control for a
# bounded window and logs free decay. Duration is randomised within this
# range per run (varied thermal-loss conditions across runs build a richer
# dataset than always testing at the same duration).
DEEP_CALIBRATION_MIN_MIN: float = 20.0
DEEP_CALIBRATION_MAX_MIN: float = 40.0

# Live Actuator Response Testing (7.4) — nudges a thermostat-controlled
# zone's setpoint by this amount to measure time-to-target, staying well
# within the existing safety envelope.
LIVE_TEST_SETPOINT_NUDGE_C: float = 1.0
# Cap on how long a setpoint-nudge test waits for the zone to reach target
# before giving up and recording whatever was observed — a real fault
# (stuck actuator, sensor dropout) must not hang a test indefinitely.
LIVE_TEST_MAX_WAIT_MIN: float = 30.0

# Passive logging (7.5) — hourly aggregation, matching journal_store.py's
# Store-based persistence pattern (see learning_store.py).
LEARNING_LOG_INTERVAL_MIN: float = 60.0

# Optional InfluxDB (or InfluxDB-line-protocol-compatible, e.g.
# VictoriaMetrics) export (8.2) — off by default, best-effort, one-way
# mirror. Never a dependency of the core learning system.
